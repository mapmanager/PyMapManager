...
class AnnotationsLayers(AnnotationsInteractions):
    ...
    def _getSpines(self, options: AnnotationsOptions) -> list[Layer]:
        ...
        if editing:
            ...
            # logger.info(f"editingSegmentId {type(editingSegmentId)}")
            # temp = self._points["segmentID"]
            ...
            #     elif val is False:
            #         logger.info(f"index of false is: {i}")

            not_faded = self._points[visible_mask]["z"].between(
                zRange[0], zRange[1])
            
            # logger.info(f"editing visible_mask {visible_mask}")
        else:
        ...
        if len(points) == 0:
            logger.info(f"returning since there are 0 points")
            return layers
    ...
    def _segmentGhost(self, segId, showLineSegmentsRadius, layers, segment, boarderWidth, offset):
        ...
        segId = int(float(segId))
        logger.info(f"segId is: {segId}")
