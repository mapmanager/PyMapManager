...
class stackWidget2(mmWidget2):
    ...
    def _buildUI(self):
        ...
        self._widgetDict = {}  # keep a dict of widgets to toggle visible
        ...
        pointListDock = QtWidgets.QDockWidget('Points')
        pointListDock.setWidget(plw)
        pointListDock.setFeatures(QtWidgets.QDockWidget.NoDockWidgetFeatures)
        pointListDock.setFloating(False)
        self.addDockWidget(QtCore.Qt.LeftDockWidgetArea, pointListDock)
        ...
        lineListDock = QtWidgets.QDockWidget('Lines')
        lineListDock.setWidget(llw)
        lineListDock.setFeatures(QtWidgets.QDockWidget.NoDockWidgetFeatures)
        lineListDock.setFloating(False)
        self.addDockWidget(QtCore.Qt.LeftDockWidgetArea, lineListDock)
        ...
        _imagePlotWidget = ImagePlotWidget(self)
                    # self._stack,
                    # contrastDict = self._contrastDict,
                    # colorLutDict = self._colorLutDict,
                    # displayOptionsDict = self._displayOptionsDict,
                    # name = imagePlotName,
                    # stackWidgetParent = self,
                    # parent=None)
        hBoxLayout_main.addWidget(_imagePlotWidget)
        self._widgetDict[imagePlotName] = _imagePlotWidget  # the dock, not the widget ???

        #
        # histogramName = HistogramWidget._widgetName
        # self._histogramWidget = HistogramWidget(self)
        #                                         # self._stack,
        #                                         # self._contrastDict,
        #                                         # name = histogramName,
        #                                         # pmmParentWidget=self)
        # histogramDock = QtWidgets.QDockWidget('Histogram')
        # histogramDock.setWidget(self._histogramWidget)
        # histogramDock.setFeatures(QtWidgets.QDockWidget.NoDockWidgetFeatures)
        # histogramDock.setFloating(False)
        # self.addDockWidget(QtCore.Qt.BottomDockWidgetArea, histogramDock)
        # self._widgetDict[histogramName] = histogramDock  # the dock, not the widget ???
