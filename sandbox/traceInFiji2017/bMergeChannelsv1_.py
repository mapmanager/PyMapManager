...
Purpose: Open an ROI Manager and 3D view from a Map Manager stack export

How To Use:

1) In a Map Manager (Igor) stack, select main mapmanager menu
   MapManager - Experimental - Export To Fiji
2) This will create a .txt file and a .swc file in a
   MapManager_Export folder on your desktop
3) Run this plugin in Fiji and select the .txt file that was just created
4) Optionally, you can open the .swc file from within Simple Neurite Tracer

To Do:

 - Each ROI in the ROI manager has a properties-width. Set this LARGER so we can see ROIs better
...
def myRun():
	...
	for line in file.readlines():
		...
		if currLine.startswith('lPoint'):
			...
			# 20180102, keep track of largest ID
			xTmp, yTmp, zTmp, idTmp = value.split(',')
			if int(idTmp) > largestID:
				largestID = int(idTmp)
	...
	if doROI==1:
		...
		for currPoint in pointList:
			...
			# take z-slace (um) into account
			#manager.add(imp1, pROI, int(z*voxelz)) #add WITH slice, I want the pROI to be a 3D point
	...
	if do3d==1:
		...
		for tmptmp in range(largestID + 1):
			lps.append([]) #list of backbone centerline points (lPointList)
		...
		for currPoint in lPointList:
			...
			if currID != lastID:
				...
				#tmpItemsInList += 1
			#p.scale(cal.pixelWidth * 1/scale2D)
			# 20180102, if currID is out of order
			# append empty elements (in the end we should have all lps[:] filled in
			# lps[i] has to match actual currID
			# 20180102, this will not work when currID are out of order, e.g. 2,3,4,1
			lps[currID].append(p)
			#lps[tmpItemsInList].append(p)
