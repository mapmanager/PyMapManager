...
class PixelSource:
    ...
    def get_spines(self, options):
        ...
        # logger.info(f'self._points["point"].z: {self._points["point"].z}')
        # find visible points
        ...
        # logger.info(f'visible_points: {visible_points}')
        ...
        # logger.info(f'filtered_mask: {filtered_mask}')
        for frame in frames:
        ...
        # logger.info(f'mask frames: {roiFrames}')
...
def load_csv(path, *, index_col=None, dtype=None):
    # response = pyfetch(path)
    # csv_text = response.text()
    # csv_text_io = StringIO(csv_text)
    return pd.read_csv(path, index_col=index_col, dtype=dtype)


def load_geo_csv(path, geometry, *, index_col=None, dtype=None):
    lines = load_csv(path, index_col=index_col, dtype=dtype)
    for g in geometry:
        lines[g] = lines[g].apply(wkt.loads)
    return gp.GeoDataFrame(lines, geometry=geometry[0])


def newPixelSource(src_path):
    line_segments = load_geo_csv(src_path + "/line_segments.csv", ['segment'], index_col="segmentID", dtype={'segmentID': str})


    # logger.info(f"line_segments: {line_segments}")

    points = load_geo_csv(src_path + "/points.csv", ['point', 'anchor'], index_col="spineID", dtype={'spineID': str, 'segmentID': str})
