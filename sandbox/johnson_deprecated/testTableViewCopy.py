...
from PyQt5.QtCore import Qt, QSortFilterProxyModel, QAbstractTableModel, QModelIndex, pyqtSignal,  QRegExp 
...
class myQSortFilterProxyModel(QSortFilterProxyModel):
    def __init__(self, parent=None):
        ...
        # self.setFilterKeyColumn(0) # select the first column at the start
    ...
    def filterAcceptsRow(self, sourceRow, sourceParent):
        ...
        logger.info(f'sourceRow: {sourceRow} filterCol: {filterCol}')
        ...
        # logger.info(f'checkPattern: {checkPattern}')
                          
        checkComparisonVal = self.currentComparisonValue != ""

        # issue is that it is detecting a random value at the valIndex initially
        ...
        # if(checkPattern and self.getComparisonSymbol == "=" ):
        #     return True
...
def testDF():
    ...
    container = SearchController(df)

    return container

# TODO: make test cases that activate each combination
