...
from pymapmanager.interface.mapWidgets.mapTableWidget import mapTableWidget
...
class mapWidget(MainWindow):
    ...
    def zoomToPointAnnotation(self,
        ...
        logger.warning('mapWidget() does not have snap to point, stackWidget() has that!')
        return
    ...
    def openStack2(self, session : int, posRect : List[int] = None) -> stackWidget:
    ...
    def openStack(self,
        ...
        bsw = stackWidget(timeseriescore=self._map,
                                                        mapWidget=self,
                                                        timepoint=session)
    ...
    def _buildUI(self):
        ...
        # dendrogramWidget = pmm.interface.mapWidgets.dendrogramWidget(self)
        
        # to hide from circular imports
        # logger.info('turned off dendrogram widget to focus on single timepoints')
        if 0:
            from pymapmanager.interface.mapWidgets.mapDendrogramWidget import MapDendrogramWidget
